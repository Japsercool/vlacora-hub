...
export type MusicMeetingTrack={
  id:string;meetingId:string;source:"manual";sourceSongId:string|null;artist:string;title:string;category:string;
  musicFolder:string;audioUrl:string;position:number;decision:string;note:string;addedBy:string|null;createdAt:string;updatedAt:string;
};
...
export type MeetingTrackInput={
  source:"manual";sourceSongId?:string|null;artist:string;title:string;category?:string;musicFolder?:string;audioUrl?:string;
};
...
function mapTrack(r:any):MusicMeetingTrack{return{id:String(r.id),meetingId:String(r.meeting_id),source:"manual",sourceSongId:r.source_song_id?String(r.source_song_id):null,artist:String(r.artist||""),title:String(r.title||""),category:String(r.category||""),musicFolder:String(r.music_folder||""),audioUrl:String(r.audio_url||""),position:Number(r.position||0),decision:String(r.decision||""),note:String(r.note||""),addedBy:r.added_by?String(r.added_by):null,createdAt:String(r.created_at),updatedAt:String(r.updated_at)}}
...
export async function addMusicMeetingTracks(meetingId:string,tracks:MeetingTrackInput[]){
  ...
  const rows=tracks.map(track=>({meeting_id:meetingId,source:track.source,source_song_id:track.sourceSongId||null,artist:track.artist.trim(),title:track.title.trim(),category:track.category||"",music_folder:track.musicFolder||"",audio_url:track.audioUrl||"",position:pos++,added_by:uid}));
...
export async function updateMusicMeetingTrack(track:MusicMeetingTrack,patch:Partial<Pick<MusicMeetingTrack,"decision"|"note"|"position"|"artist"|"title"|"category"|"musicFolder">>){
  const payload:any={updated_at:new Date().toISOString()};
  ...
  if(patch.musicFolder!==undefined)payload.music_folder=patch.musicFolder;
