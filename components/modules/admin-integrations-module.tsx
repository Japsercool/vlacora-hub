...
import { createHubStation,deleteHubStation,hydrateHubStations,readAllHubStations,saveHubStation,type HubStation } from "@/lib/hub-stations";
...
export default function AdminIntegrationsModule({stationName,stationSlug}:{stationName:string;stationSlug:string}){
  ...
  async function addStation(){
    ...
    try{const slug=await createHubStation({name:newName,short:newShort});setNewName("");setNewShort("");await refreshStations();flash("Zender aangemaakt in Supabase/PostgreSQL");router.push(`/hub/${slug}/beheer`)}catch(e){flash(e instanceof Error?e.message:"Zender aanmaken mislukt")}finally{setBusy(false)}
  }
  ...
  return <div>
    ...
    {stationSlug==="all"&&<>
      ...
      <section className="card"><div className="section-head"><div><h3>Database-backend</h3><p>Supabase is nu actief. De HUB bewaart alvast de migratiedoelconfiguratie zodat een toekomstige overstap gecontroleerd kan gebeuren.</p></div><span className="badge badge-blue">SUPABASE / POSTGRESQL</span></div>
        <div className="database-current"><div><span>Actieve backend</span><strong>Supabase PostgreSQL</strong><small>Authenticatie + PostgreSQL + RLS</small></div><div><span>Omschakeling</span><strong>Voorbereid</strong><small>Databasewachtwoorden worden nooit in de browser opgeslagen.</small></div></div>
