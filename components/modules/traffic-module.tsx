...
export default function TrafficModule({stationSlug}:{stationSlug:string}){
  ...
  return <div className="traffic-page">
    ...
    <section className="card traffic-list-card">
      ...
      <div className="traffic-list">{snapshot?.items.map(item=><article key={item.id} className={`traffic-item severity-${item.severity}`}><span className="traffic-severity-dot"/><div className="traffic-item-main"><div className="traffic-item-meta"><b>{item.road||item.location||"Verkeersmelding"}</b><span>{item.typeLabel}</span>{item.direction&&<span>richting {item.direction}</span>}</div><strong>{item.summary}</strong><small>Bijgewerkt {fmtDateTime(item.updatedAt)}{item.validUntil?` • geldig tot ${fmtDateTime(item.validUntil)}`:""}</small></div></article>)}{snapshot&&!snapshot.items.length&&<div className="empty-live-state compact"><strong>Geen relevante meldingen</strong><span>Voor de gekozen wegen zijn momenteel geen grote meldingen gevonden.</span></div>}</div>
    </section>
  </div>;
}
