...
export default function ContentInboxModule({stationSlug}:{stationSlug:string}){
  ...
  async function submit(){
    ...
    try{const created=await createContentItem({stationSlug,contentType:type,title,description,targetProgramId:target||null});if(newFiles.length)await uploadAttachments(stationSlug,"content_inbox",String(created.id),newFiles);setTitle("");setDescription("");setTarget("");setNewFiles([]);await load();flash("Ingestuurd naar de content-inbox")}catch(e){flash(e instanceof Error?e.message:"Insturen mislukt")}
  }
