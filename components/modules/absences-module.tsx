    ...
    <div className="absence-layout">
      ...
      <section className="absence-list">
        ...
        {upcoming.map(item=><article className="card absence-card" key={item.id}>
          <div className="absence-card-head"><div className="absence-avatar">{item.userName.split(/\s+/).map(x=>x[0]).slice(0,2).join("").toUpperCase()}</div><div><h3>{item.userName} afwezig</h3><p>{fmtDate(item.startsOn)} → {fmtDate(item.endsOn)}{item.reason?` • ${item.reason}`:""}</p></div><div className="absence-head-actions"><span>{item.coverages.filter(x=>x.status==="confirmed").length}/{item.coverages.length} vervanging rond</span>{(admin||item.userId===collaboration.currentUser?.id)&&<button className="ghost danger-text" onClick={()=>{if(confirm("Afwezigheid verwijderen?"))void deleteAbsence(item.id).then(load)}}>Verwijder</button>}</div></div>
          {item.notes&&<div className="absence-note">{item.notes}</div>}
          <div className="absence-impact-summary"><div><span>Geraakte uitzendingen</span><strong>{item.coverages.length}</strong></div><div><span>Open taken op naam</span><strong>{item.openTasks}</strong></div><div><span>Nog zonder vervanger</span><strong>{item.coverages.filter(x=>!x.replacementUserId||x.status==="declined").length}</strong></div></div>
          <div className="absence-coverages">
            ...
            {item.coverages.map(c=><div className="coverage-row" key={c.id}><div className="coverage-date"><strong>{fmtDate(c.airDate)}</strong><span>{c.programName}</span></div><label>Vervanger<select value={c.replacementUserId||""} onChange={e=>void assign(c.id,e.target.value)}><option value="">Nog niemand</option>{team.filter(p=>p.id!==item.userId).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label>Status<select value={c.status} onChange={e=>void status(c.id,e.target.value as any)}><option value="unassigned">Niet toegewezen</option><option value="asked">Gevraagd</option><option value="confirmed">Bevestigd</option><option value="declined">Geweigerd</option></select></label><span className={`coverage-state ${c.status}`}>{c.status==="confirmed"?"✓ Rond":c.status==="asked"?"Wacht op antwoord":c.status==="declined"?"Geweigerd":"Open"}</span></div>)}
          </div>
        </article>)}
      </section>
    </div>
