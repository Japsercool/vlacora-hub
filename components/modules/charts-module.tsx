...
import HitlistBulkImportPanel,{type HitlistBulkImportFile} from "@/components/modules/hitlist-bulk-import-panel";
import HitlistUpdatesPanel from "@/components/modules/hitlist-updates-panel";
...
const entryKeys=(entry:{songId?:string;artist:string;title:string})=>[entry.songId?`id:${entry.songId}`:"",entry.artist.trim()&&entry.title.trim()?`text:${songKey(entry.artist,entry.title)}`:""].filter(Boolean);
function positionMap(entries:Array<{songId?:string;artist:string;title:string}>){const map=new Map<string,number>();entries.forEach((entry,index)=>entryKeys(entry).forEach(key=>{if(!map.has(key))map.set(key,index+1)}));return map}
function mappedPosition(map:Map<string,number>,entry:{songId?:string;artist:string;title:string}){for(const key of entryKeys(entry)){const position=map.get(key);if(position!=null)return position}return null}
function sameSong(a:{songId?:string;artist:string;title:string},b:{songId?:string;artist:string;title:string}){if(a.songId&&b.songId&&a.songId===b.songId)return true;if(!a.artist.trim()||!a.title.trim()||!b.artist.trim()||!b.title.trim())return false;return songKey(a.artist,a.title)===songKey(b.artist,b.title)}
...
export default function ChartsModule({stationSlug,stationName}:{stationSlug:string;stationName:string}){
  ...
  const[memoryPickerId,setMemoryPickerId]=useState<string|null>(null);
  ...
  function historyFor(entries:HitlistEntry[],chart:Hitlist){
    const history=seriesEditions(chart);const prev=previousFor(chart);const prevMap=positionMap(prev?.entries||[]);const historyMaps=history.map(edition=>positionMap(edition.entries));
    return entries.map((entry,index)=>{
      const positions=historyMaps.map(map=>mappedPosition(map,entry)).filter((pos):pos is number=>pos!=null);
      return {...entry,previousPosition:mappedPosition(prevMap,entry),weeks:positions.length+1,peak:Math.min(index+1,...positions)};
    });
  }
  function recalculateHistories(all:Hitlist[]){
    ...
    for(const chart of sorted){
      const key=chart.seriesKey||seriesKeyFor(chart.name);const priors=done.filter(c=>(c.seriesKey||seriesKeyFor(c.name))===key).sort((a,b)=>(b.publishDate||b.validFrom||"").localeCompare(a.publishDate||a.validFrom||""));const explicit=chart.previousEditionId?done.find(c=>c.id===chart.previousEditionId):null;const prev=explicit||priors[0]||null;const prevMap=positionMap(prev?.entries||[]);const priorMaps=priors.map(prior=>positionMap(prior.entries));
      const entries=chart.entries.map((entry,index)=>{const positions=priorMaps.map(map=>mappedPosition(map,entry)).filter((pos):pos is number=>pos!=null);return{...entry,previousPosition:mappedPosition(prevMap,entry),weeks:positions.length+1,peak:Math.min(index+1,...positions)}});done.push({...chart,entries})
    }
  ...
  function addBlankEntry(openPicker=false){
    if(!selected)return;if(selected.entries.length>=selected.size)return flash(`Top ${selected.size} is al volledig.`);
    const id=uid();setEntries([...selected.entries,{id,artist:"",title:"",previousPosition:null,weeks:1,peak:selected.entries.length+1,notes:""}]);
    if(openPicker){setMemoryPickerId(id);setMemorySearch("")}
  }
  function chooseMemory(entryId:string,key:string){
    const song=memoryOptions.find(x=>x.key===key);if(!song||!selected)return;
    if(selected.entries.some(entry=>entry.id!==entryId&&sameSong(entry,song)))return flash("Deze song staat al in de hitlijst.");
    updateEntry(entryId,{artist:song.artist,title:song.title,songId:song.songId||undefined});setMemoryPickerId(null);setMemorySearch("");
  ...
  async function parseExcelRows(file:File){
    const XLSX=await import("xlsx");const wb=XLSX.read(await file.arrayBuffer(),{type:"array"});let parsed:Array<{rank:number;artist:string;title:string}>=[];
    for(const name of wb.SheetNames){const matrix=XLSX.utils.sheet_to_json(wb.Sheets[name],{header:1,defval:""}) as any[][];let header=-1,rankCol=-1,artistCol=-1,titleCol=-1;
      for(let r=0;r<Math.min(matrix.length,30);r++){const cells=(matrix[r]||[]).map((x:any)=>normalized(String(x)));rankCol=cells.findIndex(x=>["dw","positie","pos","rank","ranking","nummer","nr"].includes(x));artistCol=cells.findIndex(x=>["artiest","artist","uitvoerder"].includes(x));titleCol=cells.findIndex(x=>["titel","title","song","track"].includes(x));if(rankCol>=0&&artistCol>=0&&titleCol>=0){header=r;break}}
      if(header<0)continue;const rows:Array<{rank:number;artist:string;title:string}>=[];for(let r=header+1;r<matrix.length;r++){const row=matrix[r]||[];const rank=Number(String(row[rankCol]??"").replace(/[^0-9]/g,""));const artist=String(row[artistCol]??"").trim(),title=String(row[titleCol]??"").trim();if(rank>=1&&artist&&title)rows.push({rank,artist,title})}if(rows.length>parsed.length)parsed=rows
    }
    if(!parsed.length)throw new Error(`Geen hitlijsttabel gevonden in ${file.name}. Verwacht kolommen Positie/DW, Artiest en Titel.`);return parsed.sort((a,b)=>a.rank-b.rank)
  }
  async function importHistoricalWeeks(items:HitlistBulkImportFile[]){
    if(!selected)throw new Error("Open eerst de hitlijstreeks waarin je de historische weken wilt importeren.");
    setBusy(true);try{const incoming:Hitlist[]=[];for(const item of items){if(!item.week)continue;const parsed=await parseExcelRows(item.file);const entries:HitlistEntry[]=parsed.slice(0,selected.size).map(row=>({id:uid(),artist:row.artist,title:row.title,previousPosition:null,weeks:1,peak:row.rank,notes:""}));const existing=charts.find(c=>(c.seriesKey||seriesKeyFor(c.name))===(selected.seriesKey||seriesKeyFor(selected.name))&&c.editionYear===item.year&&c.editionWeek===item.week);incoming.push({...(existing||selected),id:existing?.id||uid(),name:selected.name,editionLabel:`Week ${item.week} • ${item.year}`,publishDate:existing?.publishDate||today(),validFrom:existing?.validFrom||"",validTo:existing?.validTo||"",status:existing?.status||"published",previousEditionId:"",entries,seriesKey:selected.seriesKey||seriesKeyFor(selected.name),editionYear:item.year,editionWeek:item.week,sourceLabel:`Excel • ${item.filename}`,createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()})}
      const incomingIds=new Set(incoming.map(x=>x.id));const merged=[...charts.filter(x=>!incomingIds.has(x.id)),...incoming];const recalculated=recalculateHistories(merged);setCharts(recalculated);setShowHistoricalImport(false);void rememberSongs(incoming.flatMap(c=>c.entries.map(e=>({artist:e.artist,title:e.title,songId:e.songId}))));flash(`${incoming.length} week${incoming.length===1?"":"en"} geïmporteerd • volledige historiek automatisch herberekend`)
  ...
  const duplicateCount=useMemo(()=>{if(!selected)return 0;const seen=new Set<string>();let d=0;selected.entries.forEach(e=>{if(!e.artist.trim()||!e.title.trim())return;const k=songKey(e.artist,e.title);if(seen.has(k))d++;seen.add(k)});return d},[selected]);
  ...
  const selectedPrevious=selected?previousFor(selected):null;
    ...
    <div className="chart-v11-layout">
      ...
      <div className="chart-v11-main">
        {!selected?<div className="card"><div className="empty-live-state"><strong>Maak je eerste hitlijst</strong><span>Daarna verschijnt hier de volledige rangschikking.</span></div></div>:<>
          ...
          {showHistoricalImport&&<HitlistBulkImportPanel onImport={importHistoricalWeeks}/>}
          <HitlistUpdatesPanel hitlistId={selected.id} stationSlug={stationSlug}/>
          ...
          <details className="card chart-settings-card chart-settings-details"><summary><div><strong>Editie-instellingen</strong><small>Naam, type, programma, geldigheid en vorige editie</small></div><span>Open instellingen ▾</span></summary><div className="chart-settings-grid">
            ...
            <label className="field">Publicatiedatum<input type="date" className="input" value={selected.publishDate||""} onChange={e=>patch({publishDate:e.target.value})}/></label><label className="field">Vorige editie<select className="select" value={selected.previousEditionId||""} onChange={e=>{const next={...selected,previousEditionId:e.target.value};const entries=historyFor(selected.entries,next);setCharts(recalculateHistories(charts.map(c=>c.id===selected.id?{...next,entries,updatedAt:new Date().toISOString()}:c)))}}><option value="">Automatisch • {previousFor(selected)?.editionLabel||"geen eerdere editie in deze reeks"}</option>{orderedCharts.filter(c=>c.id!==selected.id).map(c=><option value={c.id} key={c.id}>{c.name} • {c.editionLabel}</option>)}</select></label>
          ...
          <div className="card table-card chart-table-card"><div className="module-title-row chart-sheet-head"><div><h3>Hitlijstwerkblad</h3><small>Werk rechtstreeks zoals in een werkblad. Typ zelf of kies een song uit het centrale PULSE-songgeheugen.</small></div><div className="chart-sheet-tools"><span className="chart-history-status"><b>Vorige editie</b>{selectedPrevious?`${selectedPrevious.editionLabel} • automatisch gekoppeld`:"Nog geen eerdere editie in deze reeks"}</span><strong>{selected.entries.length}/{selected.size}</strong></div></div><div className="chart-sheet-toolbar"><button className="ghost" onClick={()=>addBlankEntry(true)}>＋ Song kiezen</button><button className="ghost" onClick={()=>addBlankEntry(false)}>＋ Lege rij</button><label className="ghost file-button">⇧ Excel import<input type="file" accept=".xlsx,.xls" hidden onChange={e=>void importExcel(e.target.files?.[0])}/></label><button className="ghost" onClick={()=>setShowBulk(!showBulk)}>▤ Plakken uit Excel</button><button className="ghost" onClick={nextEdition}>↗ Nieuwe editie uit deze lijst</button></div>{showBulk&&<div className="chart-bulk-box chart-bulk-inline"><textarea className="input textarea" value={bulkText} onChange={e=>setBulkText(e.target.value)} placeholder={'ANOTR\tTalk To You\nBebe Rexha\tSad Girls\nHUGEL - Movin To The Sun'}/><div><small>Plak twee kolommen (Artiest + Titel) uit Excel of regels met “Artiest - Titel”. Dubbele songs worden overgeslagen.</small><div className="button-row"><button className="ghost" onClick={()=>setShowBulk(false)}>Annuleren</button><button className="primary" onClick={()=>{addBulk();setShowBulk(false)}}>Toevoegen</button></div></div></div>}
            {selected.entries.length===0?<div className="empty-live-state"><strong>Deze editie is leeg</strong><span>Voeg een lege rij toe, kies een onthouden song of importeer/plak rechtstreeks uit Excel.</span><button className="primary soft" onClick={()=>addBlankEntry(false)}>＋ Eerste rij</button></div>:<div className="chart-table-scroll chart-sheet-scroll"><table className="chart-editor-table chart-sheet chart-sheet-v2"><thead><tr><th>#</th><th>Song</th><th>Vorige</th><th>Trend</th><th>Weken</th><th>Peak</th><th>Notitie</th><th></th></tr></thead><tbody>{selected.entries.map((e,i)=><tr key={e.id} onDragOver={ev=>ev.preventDefault()} onDrop={()=>{if(dragIndex!=null)move(dragIndex,i);setDragIndex(null)}} className={dragIndex===i?"dragging":""}><td className="chart-rank"><span className="drag-handle" draggable onDragStart={ev=>{ev.stopPropagation();setDragIndex(i)}} onDragEnd={()=>setDragIndex(null)} title="Sleep om positie te veranderen">⋮⋮</span><strong>{i+1}</strong></td><td className="chart-song-workcell"><div className="chart-song-editor"><label><span>Artiest</span><input className="chart-cell-input" value={e.artist} onChange={ev=>updateEntry(e.id,{artist:ev.target.value,songId:undefined})} onBlur={()=>{if(e.artist&&e.title)void rememberSongs([{artist:e.artist,title:e.title,songId:e.songId}])}} placeholder="Artiest"/></label><label><span>Titel</span><input className="chart-cell-input" value={e.title} onChange={ev=>updateEntry(e.id,{title:ev.target.value,songId:undefined})} onBlur={()=>{if(e.artist&&e.title)void rememberSongs([{artist:e.artist,title:e.title,songId:e.songId}])}} placeholder="Titel"/></label><button type="button" className={`chart-memory-trigger ${memoryPickerId===e.id?"active":""}`} onClick={()=>{setMemoryPickerId(memoryPickerId===e.id?null:e.id);setMemorySearch("")}} title="Kies uit onthouden songs">⌄</button></div>{memoryPickerId===e.id&&<div className="chart-memory-popover"><div className="chart-memory-search"><input autoFocus className="input" value={memorySearch} onChange={ev=>setMemorySearch(ev.target.value)} placeholder="Zoek song of artiest…"/><button className="mini-btn" onClick={()=>{setMemoryPickerId(null);setMemorySearch("")}}>×</button></div><div className="chart-memory-results">{memoryOptions.length===0?<span>Geen onthouden songs gevonden.</span>:memoryOptions.slice(0,80).map(song=><button type="button" key={song.key} onClick={()=>chooseMemory(e.id,song.key)}><span><strong>{song.title}</strong><small>{song.artist}</small></span><em>{song.source}{song.useCount?` • ${song.useCount}×`:""}</em></button>)}</div></div>}</td><td>{e.previousPosition==null?<span className="new-chip">NEW</span>:e.previousPosition}</td><td className={trendText(e,i).startsWith("▲")?"positive":trendText(e,i).startsWith("▼")?"negative":""}>{trendText(e,i)}</td><td>{e.weeks}</td><td>{e.peak}</td><td><input className="chart-cell-input note" value={e.notes} onChange={ev=>updateEntry(e.id,{notes:ev.target.value})} placeholder="optioneel"/></td><td><div className="chart-row-actions"><button className="mini-btn" disabled={i===0} onClick={()=>move(i,i-1)}>↑</button><button className="mini-btn" disabled={i===selected.entries.length-1} onClick={()=>move(i,i+1)}>↓</button><button className="mini-btn" title="Update / privé-notitie" onClick={()=>setUpdateEntryId(updateEntryId===e.id?null:e.id)}>✎</button><button className="mini-btn danger" onClick={()=>removeEntry(e.id)}>×</button></div></td></tr>)}</tbody></table><button className="chart-add-row" onClick={()=>addBlankEntry(false)} disabled={selected.entries.length>=selected.size}>＋ Nieuwe rij onderaan</button></div>}
          </div>

          {updateEntryId&&selected.entries.find(e=>e.id===updateEntryId)&&(()=>{const e=selected.entries.find(x=>x.id===updateEntryId)!;return <HitlistUpdatesPanel hitlistId={selected.id} stationSlug={stationSlug} songKey={songKey(e.artist,e.title)} entryPosition={selected.entries.indexOf(e)+1}/>})()}
