...
export default function PersonalInboxModule({stationSlug}:{stationSlug:string}){
  ...
  const[data,setData]=useState<any>({tasks:[],requests:[],meetings:[],calendar:[],replacements:[],warnings:[]});
  ...
  return <div className="personal-inbox-page">
    <div className="page-intro"><div><span className="eyebrow">PERSOONLIJKE WERKBAK</span><h2>Voor mij</h2><p>Alles wat jouw aandacht vraagt, over modules heen: taken, berichten, aanvragen, agenda, programma’s en vervangingen.</p></div><button className="ghost" onClick={()=>void load()}>↻ Vernieuw</button></div>
    <div className="personal-inbox-metrics">
      ...
      <button onClick={()=>router.push(`/hub/${stationSlug}/kalender`)}><strong>{data.calendar.length}</strong><span>komende agenda-items</span></button>
    ...
    <div className="personal-inbox-grid">
      ...
      <section className="card"><div className="section-head"><div><h3>Komende afspraken</h3><p>Je agenda, muziekmeetings en vervangingen.</p></div><button className="ghost" onClick={()=>router.push(`/hub/${stationSlug}/kalender`)}>Agenda</button></div>{data.calendar.slice(0,5).map((a:any)=><button className="personal-inbox-row" key={`cal-${a.id}`} onClick={()=>router.push(`/hub/${stationSlug}/kalender`)}><span>□</span><div><strong>{a.title}</strong><small>{new Date(a.starts_at).toLocaleString("nl-BE")}{a.location?` • ${a.location}`:""}</small></div><b>›</b></button>)}{data.meetings.slice(0,3).map((m:any)=><button className="personal-inbox-row" key={m.id} onClick={()=>router.push(`/hub/${m.station_slug}/meetings`)}><span>◎</span><div><strong>{m.title}</strong><small>{new Date(m.scheduled_at).toLocaleString("nl-BE")}</small></div><b>›</b></button>)}{data.replacements.slice(0,3).map((r:any)=><button className="personal-inbox-row" key={r.id} onClick={()=>router.push(`/hub/${stationSlug}/afwezigheden`)}><span>↔</span><div><strong>Vervanging • {r.programName}</strong><small>{r.air_date} • {r.status}</small></div><b>›</b></button>)}{data.calendar.length+data.meetings.length+data.replacements.length===0&&<div className="empty-live-state compact"><strong>Niets gepland</strong></div>}</section>
