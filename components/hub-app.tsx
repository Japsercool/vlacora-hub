    ...
    <div className="hub-shell">
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          {moduleSlug === "muziek" && <>
            <div className="page-intro"><div><h2>Nieuwe muziek inbox</h2><p>{allTracks.length} tracks in deze demo.</p></div><button className="primary" onClick={()=>setModal("track")}>+ Track toevoegen</button></div>
            <div className="music-grid">{allTracks.map((track,i)=>{const key=`${track.artist}-${track.title}`;return <Card className="music-card" key={track.id}><div className={`cover cover-${(i%4)+1}`}>♫</div><div className="music-info"><Badge tone="blue">Te bespreken</Badge><h3>{track.artist}</h3><p>{track.title}</p><small>{track.genre} • release {track.release}</small><div className="vote-row"><span>Jouw score:</span>{[6,7,8,9,10].map(n=><button onClick={()=>{setVotes({...votes,[key]:n});notify(`Score ${n}/10 opgeslagen`)}} className={votes[key]===n?"vote selected":"vote"} key={n}>{n}</button>)}</div></div><button className="ghost" onClick={()=>notify(`Preview: ${track.artist} – ${track.title} (audio volgt via Agent)`) }>▶ Preview</button></Card>})}</div>
          </>}
