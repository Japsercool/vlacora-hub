...
export default function AccountWidget(){
  ...
  useEffect(()=>{const ok=isSupabaseBrowserConfigured();setConfigured(ok);if(!ok)return;createClient().auth.getUser().then(({data})=>setEmail(data.user?.email||"VLACORA gebruiker"))},[]);
