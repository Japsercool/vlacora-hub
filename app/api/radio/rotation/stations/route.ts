...
export async function GET(request:NextRequest) {
  ...
  try{
    const path=process.env.ROTATION_ONE_STATIONS_PATH||"/api/v1/stations";
    const res=await radioFetch("rotation",path);
    ...
    return NextResponse.json({ok:false,error:error instanceof Error?error.message:"Rotation station discovery failed"},{status:502});
  }
}
